...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, d_model, num_patch, patch_len,
                 n_layers=3,  n_heads=16, shared_embedding=True,
    ...
    def forward(self, x) -> Tensor:          
        ...
        z = z[:,:,:,-1]             # only consider the last item in the sequence, x: bs x nvars x d_model
        # z = self.flatten(z)         # x: bs x nvars * d_model
        # z = self.dropout(z)
        return z
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self, src:Tensor, prev:Optional[Tensor]=None,key_padding_mask:Optional[Tensor]=None):
        ...
        ## Multi-Head attention
        if self.res_attention:
            src2, attn, scores = self.self_attn(src, src, src, prev,key_padding_mask)
        else:
